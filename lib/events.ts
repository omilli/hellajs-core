...
export function events(rootSelector: string) {
	...
	const handlers: Array<{
		...
		selector: string;
		handler: EventFn;
		delegatedHandler: EventFn;
	}> = [];

	/**
	 * Attach an event handler to elements matching the selector
	 */
	function on(eventType: string, selector: string, handler: EventFn) {
...
const eventHandlers = new Map<string, Record<string, EventFn>>();
let nextEventId = 1;
...
export function delegateEvents(
	...
	const handlers: Record<string, EventFn> = eventHandlers.get(eventId) || {};
